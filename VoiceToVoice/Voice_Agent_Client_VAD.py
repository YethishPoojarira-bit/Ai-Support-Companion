...
async def audio_input_task(session, p, audio_queue):
    ...
    Also handles "Barge-In" by monitoring volume (RMS).
    """
    print(f"🎤 Microphone active (Rate: {rate_in}, Barge-In Enabled)...")
    ...
    try:
        while True:
            ...
            # --- BARGE-IN LOGIC ---
            # Calculate Root Mean Square (RMS) amplitude to detect loud speech
            rms = audioop.rms(data, 2)
            if rms > 2000: # Threshold for speech (Increased to 2000 to avoid false interruptions)
                # If we suspect speech and queue has audio, clear it!
                if not audio_queue.empty():
                    print("🛑 User speaking! Interrupting playback...", end="\r")
